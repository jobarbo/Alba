...
console.log(window.alba);
// all input parameters are optional, they will be chosen at random if not passed into the function
function generate_composition_params(shape_type, is_moving) {
	...
	if (is_moving === undefined) {
		is_moving = weighted_choice(isMovingArr);
	}
...
var {shape_type, is_moving} = composition_params; // unpacking parameters we need in main.js and turning them into globals

window.alba.setMetadata({shape_type: shape_type, is_moving: is_moving});
...
function weighted_choice(data) {
	let rand = window.alba.prng(window.alba.params.seed);
	let total = 0;
...
//window.alba.params.seed = '0xb6370b547213e';

let {seed = window.alba.params.seed, isRenderer} = window.alba.params;

// Seed the PRNG with the input seed and use it with p5's random functions.
const prng = window.alba.prng(seed);
const pseed = parseInt(prng() * 1e15);
...
[...(pseed + 'ThxPiter')].map((c) => R((S[3] ^= c.charCodeAt() * 23205))); // seeding the random function
...
n3 = (
	...
	xi = Math.floor((x = x * s + no[(i *= 3)])), // (xi,yi,zi) = integer coordinates
	yi = Math.floor((y = y * s + no[i + 1])),
	zi = Math.floor((z = z * s + no[i + 2]))
) => (
	(x -= xi),
	(y -= yi),
...
n2 = (
	...
	yi = Math.floor(y) // (x,y) = coordinate, s = scale, i = noise offset index
) => (
	(x -= xi),
	(y -= yi),
	(x *= x * (3 - 2 * x)),
